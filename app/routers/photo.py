...
async def background_video_gen_combined(bot: Bot, chat_id: int, photo_id: str, prompt: str, model: str, user_id: int,
...
async def on_prompt(message: types.Message, state: FSMContext):
    ...
    # Используем message.bot для проброса в фоновые задачи
    if model == "kling_motion":
        ...
        task = asyncio.create_task(background_photo_gen(
