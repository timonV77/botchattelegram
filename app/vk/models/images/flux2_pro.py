...
class Flux2Pro:
    def __init__(self):
        self.model_id = "black-forest-labs/flux.2-pro"
        self.headers = {
                ...
                for _ in range(40):
                    ...
                    async with session.get(f"{BASE_URL}/media/{request_id}", headers=self.headers) as r:
                        ...
                        if status == "completed":
                            ...
                            if not final_url:
                                logging.error("❌ Flux-2 Pro completed without url. raw=%r", raw_res)
                                return None, None, None
                            return await _download_content_bytes(session, final_url)
                        if status in ("failed", "error", "cancelled"):
                            logging.error("❌ Flux-2 Pro Failed: %s | raw=%r", res.get("error"), raw_res)
                            break
            except Exception as e:
                logging.error("❌ Flux-2 Pro Exception: %s", e)
        return None, None, None
