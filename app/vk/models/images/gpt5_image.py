...
class GPT5Image:
    """Генерация изображений через OpenAI GPT-5 Image (openai/gpt-5-image)."""

    def __init__(self):
        self.model_id = "openai/gpt-5-image"
        self.headers = {
    ...
    async def generate(
        ...
        async with aiohttp.ClientSession(connector=get_connector(), timeout=timeout_config) as session:
            try:
                logging.info("🧠 GPT-5 Image Request: %s", self.model_id)

                async with session.post(f"{BASE_URL}/media", headers=self.headers, json=payload) as resp:
                    if resp.status not in (200, 201):
                        err = await resp.text()
                        logging.error("❌ GPT-5 Image Start Error: %s", err)
                    ...
                    if not request_id:
                        logging.error("❌ GPT-5 Image: не найден request_id. raw=%r", raw_data)
                        return None, None, None

                # Polling: проверка каждые 5 секунд, до ~200 секунд
                for _ in range(40):
                    ...
                    async with session.get(f"{BASE_URL}/media/{request_id}", headers=self.headers) as r:
                        ...
                        if status == "completed":
                            ...
                            if not final_url:
                                logging.error("❌ GPT-5 Image completed без url. raw=%r", raw_res)
                                return None, None, None

                            return await _download_content_bytes(session, final_url)

                        if status in ("failed", "error", "cancelled"):
                            logging.error("❌ GPT-5 Image Failed: %s | raw=%r", res.get("error"), raw_res)
                            break

            except Exception as e:
                logging.error("❌ GPT-5 Image Exception: %s", e)
