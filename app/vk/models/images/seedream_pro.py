...
class SeedreamPro:
    def __init__(self):
        self.model_id = "seedream/5-pro-text-to-image"
        self.headers = {
    ...
    async def generate(self, prompt: str, image_urls=None, quality: str = "basic", aspect_ratio: str = "1:1") -> Tuple[
        ...
        if urls:
            ...
            logging.info("Seedream Pro images_count=%s", len(payload_input["images"]))
        ...
        async with aiohttp.ClientSession(connector=get_connector(), timeout=timeout_config) as session:
            try:
                logging.info("🌊 Seedream Pro Request (Quality: %s)", quality)
                async with session.post(f"{BASE_URL}/media", headers=self.headers, json=payload) as resp:
                    if resp.status not in (200, 201):
                        logging.error("❌ Seedream Pro Error: %s", await resp.text())
                    ...
                    if not request_id:
                        logging.error("❌ Seedream Pro request_id не найден. raw=%r", raw_data)
                        return None, None, None

                # Polling: Seedream Pro довольно быстрая (проверка каждые 5 сек)
                for attempt in range(40):  # До ~200 секунд
                    ...
                    async with session.get(f"{BASE_URL}/media/{request_id}", headers=self.headers) as r:
                        ...
                        if status == "completed":
                            ...
                            if not final_url:
                                logging.error("❌ Seedream Pro completed без url. raw=%r", raw_res)
                                return None, None, None

                            return await _download_content_bytes(session, final_url)

                        if status in ("failed", "error", "cancelled"):
                            logging.error("❌ Seedream Pro Failed: %s | raw=%r", res.get("error"), raw_res)
                            break

            except Exception as e:
                logging.error("❌ Seedream Pro Exception: %s", e)
